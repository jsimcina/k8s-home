...
| `workers.yaml.j2`                       | Worker-only documents (does not exist yet; all nodes are control-plane) |
| `nodes/<role>/<node>.yaml.j2`           | Per-node documents (currently just hostname)                            |
| `nodes/<role>/<node>.schematic.yaml.j2` | Optional per-node schematic override (none exist today)                 |
